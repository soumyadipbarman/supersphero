 ...
 unsigned int length = (unsigned int) Object_4v.size();
  ...
  if (nin>=nmnjet){
   ...
   //cout<<"ceny3==="<<ceny3<<"  ceny3wexp==="<<ceny3wexp<<"  ceny3wr=="<<ceny3wr<<endl;
    ...
    //  }
//--------------------------------------------------------------------------------------------
   //Suman Kundu Added SuperSphero(S6)
   double cen_sumPT_up =0.0;
   double cen_sumPT_down=0.0;
   double cen_rap_up=0.0;
   double cen_rap_down=0.0;

   double cen_phi_up=0.0;
   double cen_phi_down=0.0;
   for(unsigned int ij=0; ij<nin; ij++){
      dot_product_b =Object_Px_in[ij]*ThrustAxis[0]+Object_Py_in[ij]*ThrustAxis[1];
      if(dot_product_b>=0){
       cen_sumPT_up+=Object_Pt_in[ij];
       cen_rap_up+=Object_Pt_in[ij]*Object_Eta_in[ij];
       cen_phi_up+=Object_Pt_in[ij]*Object_Eta_in[ij];
      }else{
       cen_sumPT_down+=Object_Pt_in[ij];
       cen_rap_down+=Object_Pt_in[ij]*Object_Eta_in[ij];
       cen_phi_down+=Object_Pt_in[ij]*Object_Eta_in[ij];
      }
     }

    cen_rap_up= cen_rap_up/cen_sumPT_up;
    cen_phi_up= cen_phi_up/cen_sumPT_up;

    cen_rap_down = cen_rap_down/cen_sumPT_down;
    cen_phi_down = cen_phi_down/cen_sumPT_down;

    //What is Q_Perp ?
    double Q_perp_up =0.0; //Q perpendicular
    double Q_perp_down =0.0; //Q perpendicular
    for(unsigned int ij=0; ij<nin; ij++){
      dot_product_b =Object_Px_in[ij]*ThrustAxis[0]+Object_Py_in[ij]*ThrustAxis[1];
      if(dot_product_b>=0){
      Q_perp_up+=Object_Pt_in[ij];
       }else{ 
      Q_perp_down+=Object_Pt_in[ij];
      }
    }
    double MU11=0, MU12=0, MU21=0, MU22=0;
    double MD11=0, MD12=0, MD21=0, MD22=0;
// Create the Tensor M for each Half
   for(unsigned int ij=0; ij<nin; ij++){
      dot_product_b =Object_Px_in[ij]*ThrustAxis[0]+Object_Py_in[ij]*ThrustAxis[1];
      if(dot_product_b>=0){
      double delta_eta_up = Object_Eta_in[ij]-cen_rap_up;
      double delta_phi_up = DeltaPhi(Object_Phi[ij],cen_phi_up); 
      MU11 += Object_Pt_in[ij]*delta_eta_up*delta_eta_up/Q_perp_up;
      MU12 += Object_Pt_in[ij]*delta_eta_up*delta_phi_up/Q_perp_up;
      MU21 += Object_Pt_in[ij]*delta_eta_up*delta_phi_up/Q_perp_up;
      MU22 += Object_Pt_in[ij]*delta_phi_up*delta_phi_up/Q_perp_up;
      }else{
      double delta_eta_Dn= Object_Eta_in[ij]-cen_rap_down;
      double delta_phi_Dn= DeltaPhi(Object_Phi[ij],cen_phi_down);  
      MD11 += Object_Pt_in[ij]*delta_eta_Dn*delta_eta_Dn/Q_perp_down;
      MD12 += Object_Pt_in[ij]*delta_eta_Dn*delta_phi_Dn/Q_perp_down;
      MD21 += Object_Pt_in[ij]*delta_eta_Dn*delta_phi_Dn/Q_perp_down;
      MD22 += Object_Pt_in[ij]*delta_phi_Dn*delta_phi_Dn/Q_perp_down;	
     }
   }
// Find the eigen Value for each case
    double b2m4ac_up = sqrt((MU11 -MU22)*(MU11-MU22) + 4*MU12*MU21);
    double lamda_up1 = ((MU11 + MU22) + b2m4ac_up)/2.;
    double lamda_up2 = ((MU11 + MU22) - b2m4ac_up)/2.;
//
    double b2m4ac_down = sqrt((MD11 -MD22)*(MD11-MD22) + 4*MD12*MD21);
    double lamda_down1 = ((MD11 + MD22) + b2m4ac_down)/2.;
    double lamda_down2 = ((MD11 + MD22) - b2m4ac_down)/2.;

   double lamda_u2 =min(lamda_up1,lamda_up2);
   double lamda_d2 =min(lamda_down1,lamda_down2);

   double s6=min(lamda_u2,lamda_d2);
   EventShapes[33]=s6;

//---------------------------------------------------------------------
    //    for (int ij=3; ij<32; ij++) {
    for (int ij=0; ij<34; ij++) {
